...
def plot_HVAC_results(HVAC_results):
    ...
    #T = 18 #we have 20 values????
    
    Temp_r1 = np.concatenate([day['temp1'] for day in HVAC_results])
    Temp_r2 = np.concatenate([day['temp2'] for day in HVAC_results])
    h_r1    = np.concatenate([day['p1'] for day in HVAC_results])
    ...
    save_dir = "assignment/taskA/results"    
    
    save_path = os.path.join(save_dir, "HVAC_Simulation.png")
